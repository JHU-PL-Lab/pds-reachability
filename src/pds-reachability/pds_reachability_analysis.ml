...
sig
  ...
  *)
  ...
  (** Takes a step toward closing a given reachability analysis.  If the
      analysis is already closed, it is returned unchanged.  If a start state is
      found to reach another state, that pair of states will be returned in a
      lazy computation. *)
  val closure_step_reachable :
    analysis ->
    analysis * (State.t * Stack_action.t list * State.t) option Lazy.t
...
struct
  ...
  let closure_step_reachable (analysis : analysis)
    : analysis * (State.t * Stack_action.t list * State.t) option Lazy.t =
    let (new_work_collection, work_opt) =
      Work_collection_impl.take analysis.work_collection
    in
    let (result_analysis : analysis),
        (reachable_discovered :
           (State.t * Stack_action.t list * State.t) option Lazy.t) =
      match work_opt with
      | None -> analysis, lazy None
      | Some work ->
        lazy_logger `trace
          (fun () ->
        ...
        *)
        ...
        | Work.Introduce_edge edge ->
          ...
          (lazy (
             (* We only report newly-added edges which are nops leading from
                starting points. *)
             if Node_set.mem edge.source analysis'.start_nodes &&
                edge.edge_action = Nop then begin
               match edge.source, edge.target with
               | Intermediate_node(Static_destination(State_node state),
                                   actions),
                 State_node state' ->
                 Some(state,actions,state')
               | _, _ ->
                 None
             end else
               None
    ...
    logged_analysis, reachable_discovered
  ;;
